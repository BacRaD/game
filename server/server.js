...
app.get("/destionations", verifyToken, (req, res) => {
  const sql =
    "SELECT d.DestinationID, d.Destinationimage, d.Body, d.Destinationname, IF(t.TourID IS null, 0, COUNT(d.DestinationID)) AS tcount FROM destination d LEFT JOIN tours t ON d.DestinationID = d.DestinationID GROUP BY d.DestinationID ORDER BY tcount DESC;";
  con.query(sql, (err, result) => {
    if (err) throw err;
...
app.get("/names", verifyToken, (req, res) => {
  const sql =
    "SELECT p.PeopleID, p.Name, p.GroupID  FROM people p INNER JOIN groups g ON p.GroupID = g.GroupID;";
  con.query(sql, (err, result) => {
    if (err) throw new err();
    res.send(JSON.stringify(result));
  });
...
app.post("/people", verifyToken, (req, res) => {
  ...
  const sql =
    "SELECT * FROM people p INNER JOIN groups g ON p.GroupID = g.GroupID WHERE p.Name = " +
    mysql.escape(user) +
    ";";
  con.query(sql, (err, result) => {
    if (err) throw new err();
...
app.post("/tours", verifyToken, upload.single("file"), (req, res) => {
  ...
  let sql = `INSERT INTO tours(DestinationID, Pointvalue, Tourimage, Date) VALUES(${
    tour.DestinationID
  }, ${tour.PointValue}, ${mysql.escape(tour.Tourimage)}, ${mysql.escape(
    tour.Date
  )});`;
  con.query(sql, (err, result) => {
    if (err) throw err;
    ...
    for (let i = 0; i < peoples.length; i++) {
      let sql2 = `INSERT INTO tourspeople(TourID, PeopleID) VALUES (${mysql.escape(
        tourID
      )}, ${mysql.escape(peoples[i])});`;
...
app.get("/destinationimages/:id", (req, res) => {
  const sql = `SELECT t.Tourimage from tours t WHERE t.DestinationID = ${mysql.escape(
    req.params.id
  )};`;
  con.query(sql, (err, result) => {
    if (err) throw err;
